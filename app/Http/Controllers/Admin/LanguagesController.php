...
class LanguagesController extends Controller
{
    ...
    {
        $languages = Language::select()->paginate(PAGINATION_COUNT);//query scope
        return view('admin.languages.index', compact('languages'));//l table b attr elii ana a5trtha
    }
    ...
    public function store(LanguageRequest $request)
    {
        try {
            ...
            Language::create($request->except(['_token']));//except tokens
            return redirect()->route('admin.languages')->with(['success' => 'تم حفظ اللغة بنجاح']);//hab3tha b sucess l div fe layout
        } catch (\Exception $ex) {
